...
FaceDetector::FaceDetector(int _num_outputs,
                           int _num_threads,
                           int _input_height,
                           int _input_width) {
...
void FaceDetector::load_model(const std::string &_param_path, const std::string &_bin_path) {
...
void FaceDetector::initial_context() {
    if (num_outputs == 6) {
        fmc = 3;
...
void FaceDetector::resize_unscale(const cv::Mat &mat, cv::Mat &mat_rs,
...
void FaceDetector::transform(const cv::Mat &mat_rs, ncnn::Mat &in) {
    // BGR NHWC -> RGB NCHW
    in = ncnn::Mat::from_pixels(mat_rs.data, ncnn::Mat::PIXEL_BGR2RGB, input_width, input_height);
    in.substract_mean_normalize(mean_vals, norm_vals);
}

void FaceDetector::detect(const cv::Mat &mat, std::vector<types::BoxfWithLandmarks> &detected_boxes_kps,
...
void FaceDetector::generate_points(const int target_height, const int target_width) {
...
void FaceDetector::generate_bboxes_kps(const SCRFDScaleParams &scale_params,
                                       std::vector<types::BoxfWithLandmarks> &bbox_kps_collection,
...
void FaceDetector::generate_bboxes_single_stride(
...
void FaceDetector::generate_bboxes_kps_single_stride(
        const SCRFDScaleParams &scale_params, ncnn::Mat &score_pred, ncnn::Mat &bbox_pred,
...
void FaceDetector::nms_bboxes_kps(std::vector<types::BoxfWithLandmarks> &input,
